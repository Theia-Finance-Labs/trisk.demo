...
for (item in items) {
  ...
  data_grouped <- data %>%
    ...
    mutate(# Adjust 'eco' for the year 2050
      ...
      # Adjust 'prisk' for the year 2050
      prisk = ifelse(year == 2048, soc * (1 + priskfactor), prisk)
    ) %>%
    mutate(
      ...
      # Adjust 'prisk' for the years 2030-2049
      prisk = ifelse(
        year >= 2030 & year < 2048,
        prisk + (prisk[year == 2048] - net_profits_shock_scenario[year == 2048]) / 19 * (year - 2029),
        prisk
      )
    ) %>%
    ...
    mutate(
      ...
      disc_TV_prisk = TV_prisk / ((1 + discount_rate) ^ (2049 - 2022))
    )
